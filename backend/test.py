...
def get_medicine_info(medicine):
    normalized_input = normalize_text(medicine)
    
    # Find medicines that contain the given name (partial search, case insensitive)
    matches = data[data['Medicine_Name'].apply(lambda x: normalize_text(x)).str.contains(normalized_input, na=False)]
    
    if not matches.empty:
        row = matches.iloc[0]
        index = data.index[data['Medicine_Name'] == row['Medicine_Name']][0]
        
        # Get top 3 alternative medicines based on similarity
...
def search():
    ...
    medicine_name = request_data.get("medicine", "")
    response = get_medicine_info(medicine_name)
